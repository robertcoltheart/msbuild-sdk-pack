...
public class PackProjectReferenceTests
{
    ...
    [Test]
    public async Task PacksWithPackagePath()
    {
        using var fixture = new MSBuildFixture();

        var libPath = fixture.WriteReferenceProject("Lib", "<TargetFramework>netstandard2.0</TargetFramework>");
        var appPath = fixture.WriteConsumingProject("App", "<TargetFramework>net10.0</TargetFramework>", "../Lib/Lib.csproj", "PackagePath=\"dotnet/cs\"");

        var result = await fixture.Pack(appPath);

        await Assert.That(result.Success).IsTrue();
        await Assert.That(result.PackageContains("App", "lib/netstandard2.0/Lib.dll")).IsFalse();
        await Assert.That(result.PackageContains("App", "lib/net10.0/Lib.dll")).IsFalse();
        await Assert.That(result.PackageContains("App", "dotnet/cs/Lib.dll")).IsTrue();
    }

    [Test]
    public async Task PacksXmlDocumentation()
    {
        using var fixture = new MSBuildFixture();

        var libPath = fixture.WriteReferenceProject("Lib", "<TargetFramework>net10.0</TargetFramework><GenerateDocumentationFile>true</GenerateDocumentationFile>");
        var appPath = fixture.WriteConsumingProject("App", "<TargetFramework>net10.0</TargetFramework>", "../Lib/Lib.csproj");
